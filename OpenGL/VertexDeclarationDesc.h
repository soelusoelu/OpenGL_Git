#ifndef _VERTEX_DECLARATION_DESC_H_
#define _VERTEX_DECLARATION_DESC_H_

//セマンティック
enum class VertexSemantic {
    VERTEX_SEMANTIC_POSITION,
    VERTEX_SEMANTIC_BLENDWEIGHT,
    VERTEX_SEMANTIC_NORMAL,
    VERTEX_SEMANTIC_COLOR,
    VERTEX_SEMANTIC_DIFFUSE,
    VERTEX_SEMANTIC_SPECULAR,
    VERTEX_SEMANTIC_TESSFACTOR,
    VERTEX_SEMANTIC_FOGCOORD,
    VERTEX_SEMANTIC_PSIZE,
    VERTEX_SEMANTIC_BLENDINDICES,
    VERTEX_SEMANTIC_TEXCOORD,
    VERTEX_SEMANTIC_TANGENT,
    VERTEX_SEMANTIC_BINORMAL
};

//頂点タイプ
enum class VertexType {
    VERTEX_TYPE_FLOAT1,
    VERTEX_TYPE_FLOAT2,
    VERTEX_TYPE_FLOAT3,
    VERTEX_TYPE_FLOAT4,
    VERTEX_TYPE_BYTE4,
    VERTEX_TYPE_BYTE4N,
    VERTEX_TYPE_UBYTE4,
    VERTEX_TYPE_UBYTE4N,
    VERTEX_TYPE_SHORT2,
    VERTEX_TYPE_SHORT2N,
    VERTEX_TYPE_USHORT2,
    VERTEX_TYPE_USHORT2N,
    VERTEX_TYPE_SHORT4,
    VERTEX_TYPE_SHORT4N,
    VERTEX_TYPE_USHORT4,
    VERTEX_TYPE_USHORT4N,
    VERTEX_TYPE_INT1,
    VERTEX_TYPE_INT2,
    VERTEX_TYPE_INT3,
    VERTEX_TYPE_INT4,
    VERTEX_TYPE_UINT1,
    VERTEX_TYPE_UINT2,
    VERTEX_TYPE_UINT3,
    VERTEX_TYPE_UINT4
};

//頂点宣言ディスクリプタ
struct VertexDeclarationDesc {
    //セマンティック
    VertexSemantic semantic;
    //セマンティックインデックス
    unsigned index;
    //頂点タイプ
    VertexType type;
    //オフセット
    unsigned offset;
    //ストリーム番号
    unsigned stream;
};

//頂点宣言の最大値
static const unsigned VERTEX_DECLARATION_MAX = 16;

#endif
